...
| 파일 | 사이트 | 담당 |
|---|---|---|
| `us.html` | 미국법인 종합 ERP | **담당자** |
| `th.html` | 태국법인 ERP | **담당자** |
| `index.html` | us로 보내는 런처 | 오너 |

- 라이브 주소: 미국 https://wt-management.github.io/wt-corps/us.html · 태국 https://wt-management.github.io/wt-corps/th.html
...
2. **`us.html`(미국)과 `th.html`(태국)만 수정한다.** 이 두 사이트가 담당 영역이다.
   ...
   - **us.html과 th.html은 코드가 서로 갈라져 있다** → "한쪽을 참고해 다른쪽을 다시 만들기" 금지. 각 파일을 직접 고친다. (미국은 CRM 등 기능이 더 많고, 태국은 기본형이라 구조가 다름.)
...
- 저장 위치: `cons_cache` 테이블 (사이트마다 다른 key)
  - `us.html` → **key = `corp_us_erp`**
  - `th.html` → **key = `corp_th_erp`**
- 블롭 구조: `{ rate, sales, orders, items, moves, funds, clients, deals, acts, tasks, equipment, shipments, targets, accounts, snapshots }` (미국 기준. 태국은 일부 없음)
- **주의 — 마지막 저장이 이김(last-writer-wins):** 저장 시 블롭 전체를 덮어쓴다. 여러 명이 동시에 편집하면 나중 저장이 앞 저장을 지울 수 있으니, 큰 변경 전엔 팀에 알린다.
- **로컬에서 안전하게 테스트:** 파일을 localhost로 열면 LOCAL 모드가 되어 **저장이 안 되고 샘플 데이터로만** 동작한다. 마음껏 실험해도 실데이터는 안 바뀐다.
...
- 한국어 / English 토글 둘 다 안 깨지는지(사이트에 자동번역이 내장돼 있음)
- **어느 사이트를 고쳤는지 확인** — 미국이면 us.html, 태국이면 th.html. 헷갈리지 않게 한 번에 한 파일씩.
- 그 다음 커밋 → 푸시 → PR

## 자주 하는 작업 위치 (해당 html 안에서 검색)
